#!/usr/bin/env bash
# Release script for artifactory-gradle-plugin (v5 branch, Java 8).
# Runnable from GitHub Actions or directly on a developer machine.
#
# Expected env vars:
#   NEXT_VERSION                          - version to release (e.g. 5.2.1)
#   NEXT_DEVELOPMENT_VERSION               - next dev/snapshot version (e.g. 5.2.2-SNAPSHOT)
...
jf c rm --quiet
jf c add internal --url=$ARTIFACTORY_URL --user=$ARTIFACTORY_USER --access-token=$ARTIFACTORY_APIKEY
